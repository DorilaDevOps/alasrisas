...
export function renderUsers() {
  ...
  users.forEach(u => {
    ...
    const descTruncated = desc.length > 40 ? desc.substring(0, 40) + '...' : desc;
    tr.innerHTML = `
      <td>${avatarHtml}</td>
      <td>${escHtml(u.nombre)}</td>
      <td>${escHtml(u.nick || '\u2014')}</td>
      <td class="desc-cell" title="${escHtml(desc)}">${escHtml(descTruncated || '\u2014')}</td>
...
export function showUserDetail(id, isEditMode) {
  ...
  if (isEditMode) {
    ...
    modalBody.innerHTML = `
      <div class="user-detail">${avatarHtml}
        ...
        <div class="form-field">
          <label for="editDescripcion" style="font-weight:600;display:block;margin-bottom:0.25rem">Descripción</label>
    ...
    document.getElementById('saveEditUser')?.addEventListener('click', () => {
      ...
      DataService.updateUser(user.id, { nick, descripcion });
      renderUsers();
